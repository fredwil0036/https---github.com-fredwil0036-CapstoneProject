...
@include('admin.adminmodal.viewallreport')
@include('admin.adminmodal.leftnavigation')
@include('admin.adminmodal.editallreport')
                ...
                <tbody class="whitespace-nowrap">
                    @foreach ($myreports as $reports)
                      <tr class="even:bg-blue-50">
                        ...
                        <td class="p-4 text-lg">
                        {{\Carbon\Carbon::parse($reports->created_at)->format('F j Y')}}
                        </td>
                        <td class="p-4 text-lg">
                        {{\Carbon\Carbon::parse($reports->updated_at)->format('F j Y')}}
                        </td>
                        <td class="p-4 text-lg">
                        {{$reports->lowest_water_level}}
                        </td>
                        <td class="p-4 text-lg">
                        {{$reports->highest_water_level}}
                        </td>
                        <td class="p-4 text-lg">
                        <button onclick="showReportModal('{{ $reports->id }}')" class="text-white bg-gradient-to-r from-green-400 via-green-500 to-green-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">View</button>
